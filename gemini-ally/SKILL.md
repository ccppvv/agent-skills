...
**When user invokes `/gemini-ally` command:**
- ✅ ALWAYS use `gemini_bridge.py` to delegate the task to Gemini
- ❌ NEVER execute the task directly with other tools (Bash, Read, Edit, etc.)
- The user's explicit choice to use this skill means they want Gemini involvement

**Exception:** Only skip Gemini if the task is literally "show help" or "explain this skill"

## When to Use Gemini-Ally

Delegate to Gemini when:
- ✅ User explicitly invokes `/gemini-ally` (MANDATORY)
- Multi-step debugging or analysis
- Need code review from Gemini perspective
- Want to leverage Gemini's reasoning for prototyping
- Continue existing Gemini session (SESSION_ID provided)

Do NOT delegate when:
- User uses other commands/skills without explicit `/gemini-ally` call
- Simple queries about Gemini itself (help/docs)
