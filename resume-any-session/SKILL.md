...
```bash
python3 ~/.agents/skills/resume-any-session/scripts/locate_session.py <ID> --continue-here [--copy]
```
...
```bash
python3 ~/.agents/skills/resume-any-session/scripts/locate_session.py <ID> [--type codex|claude] [--json] [--copy]
```

No match? List recent sessions from both tools so the user can pick:

```bash
python3 ~/.agents/skills/resume-any-session/scripts/locate_session.py --recent
```
